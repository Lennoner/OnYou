...
export function InviteCreator() {
  ...
  const handleCreate = () => {
    if (!inviteName.trim()) {
      toast.error("초대할 친구의 이름(또는 애칭)을 입력해주세요.");
      return;
    }
    setStep("share");
  };

  const inviteLink = `https://onyou.app/invite/${Math.random().toString(36).substring(7)}`;
          ...
          {step === "create" ? (
            ...
            >
              ...
              <section className="bg-white p-8 rounded-3xl shadow-sm border border-stone-100">
                ...
                <div className={`p-6 rounded-2xl border-2 transition-colors ${inviteTemplates.find(t => t.id === selectedTemplateId)?.color}`}>
                  <textarea
                    value={customMessage}
              ...
              <button
                ...
                className="w-full py-4 bg-stone-900 text-white rounded-xl font-bold text-lg shadow-xl hover:bg-stone-800 transition-all flex items-center justify-center gap-2"
              >
                초대장 만들기 <ArrowRight size={20} />
              </button>
            </motion.div>
          ) : (
            /* Share Step */
            ...
            >
              ...
              <div className="bg-white p-8 rounded-3xl shadow-lg border border-stone-100 text-center relative overflow-hidden">
                 ...
                 <div className="grid grid-cols-2 gap-4">
                    ...
                    <button 
                      onClick={handleCopy}
                      className={`py-4 rounded-xl font-bold flex items-center justify-center gap-2 transition-all border
                        ${copied 
