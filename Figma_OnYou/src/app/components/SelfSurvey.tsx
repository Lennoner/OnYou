...
export function SelfSurvey({ onComplete }: SelfSurveyProps) {
  ...
  const updateAnswer = (value: any) => {
    ...
    } else if (currentQuestion.type === "SCALE" && currentStep === questions.length - 1) {
       // If it's the last question, just update state, let user click "Complete" or handle auto-complete if desired.
       // For safety, let's just let them review and click complete, or we could auto-complete.
       // Given "Complete" usually implies submission, maybe keep manual click for the very last step
       // or auto-show completion screen.
       // Let's stick to auto-advance logic only for non-last steps for now to avoid accidental submission.
    }
  };
